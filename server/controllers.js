...
module.exports = {
  ...
  updateTask: (req, res) => {
    const DATA = req.body;
    const ID = req.params.id;
    Ninja.findOneAndUpdate({ _id: ID }, DATA, { runValidators: true, new: true })
      .then(data => res.json(data))
      .catch(err => res.json(err))
    },
  ...
  updateSave: (req, res) => {
    const DATA = req.body;
    const ID = req.params.id;
    Save.findOneAndUpdate({ _id: ID }, { $push: { savegame: { $each: DATA } } }, 
